...
function dogFood({ fetchDogFood, testProps }) {
  ...
  return (
    <form
      className="test collapse"
      id={"feedNumber" + testProps}
      onSubmit={handleDogFoodSubmit}
    >
      <select
        className="form-control"
        onChange={(e) => setFood({ ...food, whatFood: e.target.value })}
        style={{ display: "block", width: "100%" }}
        type="text"
        placeholder="Select race"
      >
        <option defaultValue={"Select your option"} hidden>
          Select your option
        </option>
      ...
      <input
        className="form-control "
        onChange={(e) => setFood({ ...food, howMuch: e.target.value })}
        type="number"
        style={{ width: "100%" }}
        placeholder="How many grams?"
      ></input>
      <button
        className="btn btn-warning marginClass"
        style={{ display: "block", width: "100%" }}
      >
        Feed the dog!
      </button>
